Feature: Testing alma-sru-proxy in sandbox environment
  ...
  Scenario: GET Henrik Ibsen's "Samlede verker" by mms_id
    * def mms_id_value = '990600890834702201'
    ...
    And match response[0].xmlPresentation contains '<leader>03017cam a2200577 c 4500</leader>'
    And match response[0].xmlPresentation contains '<controlfield tag="001">990600890834702201</controlfield>'
    And match response[0].xmlPresentation contains '<controlfield tag="005">20260223205403.0</controlfield>'
    And match response[0].linePresentation contains '*ldr 03017cam a2200577 c 4500'
    And match response[0].linePresentation contains '001 990600890834702201'
    And match response[0].linePresentation contains '*005 20260223205403.0'
