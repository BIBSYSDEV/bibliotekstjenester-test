Feature: Testing ALMA-primo-search

  Background:
    * def BasePath = 'https://api-eu.hosted.exlibrisgroup.com/primo/v1/search'
    * def primoApiKey = PRIMO_APIKEY

  @run-pnx
  Scenario: Lookup by bibnr
    #* def recordId = 'BIBSYS_ILS71560264980002201' # TODO: Find out the new format for recordId used in the new version of Primo, use only number for now
    * def recordId = '999920556049902201'
    * def params = '?vid=47BIBSYS_NETWORK%3ABIBSYS_UNION&tab=default_tab&scope=default_scope&q=any,contains,'+ recordId +'&lang=eng'
    Given url BasePath + params
    And header Authorization = 'apikey ' + primoApiKey
    When method get
    Then status 200
    And match response.docs[0].pnx.control.recordid[0] == '#string'
    #And match response.docs[0].pnx.search.sourceid[0] == '#string' # sourceId are missing from test record
    And match response.docs[0].pnx.addata.isbn[0] == '#string'
    And match response.docs[0].pnx.addata.cop[0] == '#string'
    And match response.docs[0].pnx.addata.btitle[0] == '#string'
    #And match response.docs[0].pnx.addata.volume[0] == '#string' #testpost doesn't have volume
#    And match response.docs[0].pnx.addata.pages[0] == '#string' #testpost pages er gone
    And match response.docs[0].pnx.display.creationdate[0] == '#string'
    And match response.docs[0].pnx.display.creator[0] == '#string'
    And match response.docs[0].pnx.display.title[0] == '#string'
    And match response.docs[0].pnx.display.publisher[0] == '#string'
    #And match response.docs[0].pnx.facets.library[0] == '#string' # library are missing from test record

  @run-pnx
  Scenario: Requesting non-existing resoure
    * def recordId = 'BIBSYS_ILS000000000000000000'
    * def params = '?vid=47BIBSYS_NETWORK%3ABIBSYS_UNION&tab=default_tab&scope=default_scope&q=any,contains,' + recordId + '&lang=eng'
    Given url BasePath + params
    And header Authorization = 'apikey ' + primoApiKey
    When method get
    Then status 200
    And match response.info.total == 0







